    ...
    def build_config(self):
        ...
        if os_type in ['centos', 'rhel', 'fedora']:
            user_info['runcmd'].extend([
                ...
                #['sed', 'wtemp_keys', '/home/ubuntu/.ssh/id_rsa.pub', '/home/ubuntu/.ssh/authorized_keys'],
                #['chown', 'ubuntu:ubuntu', '/home/ubuntu/.ssh/id_rsa', '/home/ubuntu/.ssh/id_rsa.pub'],
                #['mv', 'temp_keys', '/home/ubuntu/.ssh/authorized_keys'],
                ...
                ['sed', '-ie', '/[0-3]\.centos\.pool\.ntp\.org/s/^/#/', '/etc/chrony.conf'],
                ['sed', '-ie', '3i server time0.linecorp.com iburst', '/etc/chrony.conf'],
                ['sed', '-ie', '4i server time1.linecorp.com iburst', '/etc/chrony.conf'],
                ['sed', '-ie', '5i server time2.linecorp.com iburst', '/etc/chrony.conf'],
                ['sed', '-ie', '6i server time3.linecorp.com iburst', '/etc/chrony.conf'],
                ['systemctl', 'enable', 'chronyd.service'],
                ['systemctl', 'start', 'chronyd.service'],
                ['ntpdate', 'time.linecorp.com'],
                ['sed', '-i', '-e', '$aexport PATH=$PATH:/sbin', '/etc/profile'],
                ['sed', '-i', '-e', '$aexport PATH=$PATH:/sbin', '/home/ubuntu/.bashrc'],
                ['sed', '-i', '-e', '$aMaxSessions 1000', '/etc/ssh/sshd_config'],
                ['sed', '-i', '-e', '$aAllowTcpForwarding yes', '/etc/ssh/sshd_config'],
